module lif_pipeline_formal;
  ...
  lif_pipeline dut (
    ...
    .memory_request_enable(), .memory_request_neuron(),
    .memory_voltage(memory_voltage), .memory_last_update(memory_last_update),
    .memory_threshold(memory_threshold), .memory_reset_voltage(memory_reset_voltage),
    .memory_leak(memory_leak), .commit_spike_ready(commit_spike_ready),
    .commit_valid(commit_valid), .commit_neuron(commit_neuron), .commit_tick(),
    .commit_voltage(), .commit_spike(commit_spike), .commit_accumulator_saturated(),
    .commit_membrane_saturated(), .pipeline_empty(pipeline_empty),
    .stage_valid(stage_valid), .stage_ready(stage_ready), .stage_advance(stage_advance),
    .stage_hold(stage_hold), .debug_n0_neuron(n0), .debug_n1_neuron(n1),
    .debug_n2_neuron(n2), .debug_n3_neuron(n3), .debug_n4_neuron(n4),
    .debug_n5_neuron(n5), .debug_n1_elapsed(), .debug_n2_leak_delta(),
    .debug_n2_accumulator(), .debug_n3_decay(), .debug_n3_candidate(), .debug_n4_spike()
  );
endmodule
