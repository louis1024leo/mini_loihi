...
def run_readycut_cycle_oracle(
    ...
    *,
    ...
    stall_length = spike_stall_cycles if spike_stall_length is None else spike_stall_length
    if len(program.cores) != 1:
        raise ValueError("V7.1D2 cycle oracle requires exactly one core")
    ...
    for tick in logical_tick_ids:
        ...
        cut_queue: list[dict[str, int | bool]] = []
        cut_in_ready = True
        cursor = 0
        controller = "PRE_SCAN"
        ...
        while tick_complete_cycle < 0:
            ...
            old_cut = list(cut_queue)
            old_cut_in_ready = cut_in_ready
            ...
            ready[2] = old_stages[2] is None or old_cut_in_ready
            ready[1] = old_stages[1] is None or ready[2]
            ready[0] = old_stages[0] is None or ready[1]
            ...
            dequeue = old_occupancy > 0 and _spike_ready(
                logical_cycle, spike_stall_start_cycle, stall_length, alternating_stall
            )
            enqueue = commit is not None and bool(commit["spike"])
            spike_occupancy = old_occupancy + int(enqueue) - int(dequeue)

            cut_enqueue = old_stages[2] is not None and old_cut_in_ready
            cut_dequeue = bool(old_cut) and ready[3]
            if cut_enqueue and not cut_dequeue:
                cut_in_ready = len(old_cut) < 1
            elif cut_dequeue and not cut_enqueue:
                cut_in_ready = True
            else:
                cut_in_ready = len(old_cut) < 2
            cut_queue = list(old_cut[1:] if cut_dequeue else old_cut)
            if cut_enqueue:
                cut_queue.append(old_stages[2])
            ...
            if ready[3]:
                new_stages[3] = old_cut[0] if cut_dequeue else None
            if ready[2]:
            ...
            if cut_dequeue and stages[3] is not None:
                item = stages[3]
                neuron = int(item["neuron"])
                trace.extend(
                    (
            ...
            if any(holds):
                # The RTL testbench samples the hold mask from the prior edge
                # and the debug payload after that edge.
                held = stages[5] or next(stage for stage in reversed(stages) if stage is not None)
            ...
            pipeline_empty = all(stage is None for stage in stages) and not cut_queue
            ...
            if scanner_done_at_start:
                scanner_active = False
                controller = "DRAIN"
            elif controller == "DRAIN" and all(stage is None for stage in old_stages) and not old_cut:
...
def _ingress_complete_cycle(core: object, events: tuple[ReferenceInputEvent, ...], profile: ReadyCutProfileSpec) -> int:
    request_cycle = 1
    for event in events:
...
def _spike_ready(cycle: int, start: int, length: int, alternating: bool) -> bool:
    if alternating and cycle >= start:
        return (cycle - start) % 2 == 1
    return not (start <= cycle < start + length)
