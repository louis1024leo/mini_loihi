module tb_mini_loihi_core_readycut;
  ...
  logic debug_cut_in_ready;
  logic debug_cut_out_valid;
  ...
  integer spike_stall_start;
  integer spike_stall_length;
  integer spike_stall_alternating;
  integer active_cycle_in_tick;
  integer absolute_cycle;
  integer cut_full_cycles;
  integer cut_upstream_stall_cycles;
  integer cut_maximum_occupancy;
  integer cut_pre_accept_count;
  integer cut_post_transfer_count;
  ...
  mini_loihi_core_readycut dut (
    ...
    .debug_pipeline_empty(debug_pipeline_empty),
    .debug_cut_in_ready(debug_cut_in_ready), .debug_cut_out_valid(debug_cut_out_valid),
    .debug_cut_occupancy(debug_cut_occupancy), .debug_commit_neuron(debug_commit_neuron),
  ...
  always @(posedge clk) begin
    ...
    observed_cut_dequeue <= debug_cut_out_valid && debug_pipeline_ready[3];
    if (!rst && debug_cut_occupancy == 2)
      cut_full_cycles = cut_full_cycles + 1;
    if (!rst && !debug_cut_in_ready)
      cut_upstream_stall_cycles = cut_upstream_stall_cycles + 1;
    if (!rst && debug_cut_occupancy > cut_maximum_occupancy)
      cut_maximum_occupancy = debug_cut_occupancy;
    if (!rst && debug_pipeline_valid[2] && debug_cut_in_ready)
      cut_pre_accept_count = cut_pre_accept_count + 1;
    if (!rst && debug_cut_out_valid && debug_pipeline_ready[3])
      cut_post_transfer_count = cut_post_transfer_count + 1;
  ...
  always @(negedge clk) begin
    if (!rst && debug_state != 0 && debug_state != 1 && debug_state != 2) begin
      ...
      if (spike_stall_alternating != 0 && active_cycle_in_tick > spike_stall_start)
        spike_ready = ((active_cycle_in_tick - spike_stall_start) % 2) == 0;
      else
        spike_ready = !(active_cycle_in_tick > spike_stall_start
          && active_cycle_in_tick <= spike_stall_start + spike_stall_length);
    end else begin
      spike_ready = 1'b1;
    end
  ...
  initial begin
    ...
    observed_cut_dequeue = 1'b0;
    cut_full_cycles = 0;
    cut_upstream_stall_cycles = 0;
    cut_maximum_occupancy = 0;
    cut_pre_accept_count = 0;
    cut_post_transfer_count = 0;
    ...
    spike_stall_start = 0;
    spike_stall_length = spike_stall_cycles;
    spike_stall_alternating = $test$plusargs("SPIKE_STALL_ALTERNATING");
    if (!$value$plusargs("SPIKE_STALL_START=%d", spike_stall_start)) spike_stall_start = 0;
    if (!$value$plusargs("SPIKE_STALL_LENGTH=%d", spike_stall_length))
      spike_stall_length = spike_stall_cycles;
    ...
    $display("RESULT READYCUT full_cycles=%0d upstream_stall_cycles=%0d maximum_occupancy=%0d pre_accepts=%0d post_transfers=%0d final_occupancy=%0d in_ready=%0d out_valid=%0d",
             cut_full_cycles, cut_upstream_stall_cycles, cut_maximum_occupancy,
             cut_pre_accept_count, cut_post_transfer_count, debug_cut_occupancy,
             debug_cut_in_ready, debug_cut_out_valid);
    $display("RESULT DONE");
    $finish;
  end
