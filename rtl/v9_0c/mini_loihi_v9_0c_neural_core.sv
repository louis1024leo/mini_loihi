module mini_loihi_v9_0c_neural_core #(
  ...
  output logic sample_weight_valid,
  input  logic sample_weight_ready,
  output logic [9:0] sample_weight_synapse_id,
  input  logic sample_weight_response_valid,
  input  logic signed [WEIGHT_WIDTH-1:0] sample_weight_response,
  ...
  logic base_plastic_valid_rom [0:BASE_SYNAPSE_COUNT-1];
  logic [9:0] base_plastic_id_rom [0:BASE_SYNAPSE_COUNT-1];
  logic recurrent_plastic_valid_rom [0:RECURRENT_SYNAPSE_COUNT-1];
  logic [9:0] recurrent_plastic_id_rom [0:RECURRENT_SYNAPSE_COUNT-1];
  ...
  logic [WORK_COUNT_WIDTH-1:0] external_scan_index;
  logic [WORK_COUNT_WIDTH-1:0] recurrent_scan_index;
  logic external_weight_pending;
  logic recurrent_weight_pending;
  logic [AXON_WIDTH-1:0] external_scan_axon;
  logic [PAYLOAD_WIDTH-1:0] external_scan_payload;
  logic [BASE_PTR_WIDTH-1:0] external_physical_address;
  logic [REC_PTR_WIDTH-1:0] recurrent_physical_address;
  ...
  initial begin
    ...
    $readmemh(BASE_PLASTIC_VALID_INIT, base_plastic_valid_rom);
    $readmemh(BASE_PLASTIC_ID_INIT, base_plastic_id_rom);
    $readmemh(RECURRENT_PLASTIC_VALID_INIT, recurrent_plastic_valid_rom);
    $readmemh(RECURRENT_PLASTIC_ID_INIT, recurrent_plastic_id_rom);
  end

  assign external_physical_address = axon_ptr_rom[external_scan_axon] + external_scan_index;
  assign recurrent_physical_address = recurrent_ptr_rom[
    recurrent_spikes[recurrent_memory_index]
  ] + recurrent_scan_index;
  always_comb begin
    sample_weight_valid = 1'b0;
    sample_weight_synapse_id = '0;
    if (state == STATE_EXT_SCAN && external_scan_index < work_count
        && base_plastic_valid_rom[external_physical_address]
        && !external_weight_pending) begin
      sample_weight_valid = 1'b1;
      sample_weight_synapse_id = base_plastic_id_rom[external_physical_address];
    end else if (recurrence_engine_state == REC_ENGINE_SCAN
        && recurrent_scan_index < work_count
        && recurrent_plastic_valid_rom[recurrent_physical_address]
        && !recurrent_weight_pending) begin
      sample_weight_valid = 1'b1;
      sample_weight_synapse_id = recurrent_plastic_id_rom[recurrent_physical_address];
    end
  end
  ...
  always_ff @(posedge clk) begin
    if (rst) begin
      ...
      external_scan_index <= '0;
      recurrent_scan_index <= '0;
      external_weight_pending <= 1'b0;
      recurrent_weight_pending <= 1'b0;
      external_scan_axon <= '0;
      external_scan_payload <= '0;
      active_count <= '0;
      batch_scan_index <= '0;
      ...
      if ((storage_error || pipeline_protocol_error || issue_capacity_error
        ...
        case (state)
          ...
          STATE_EXT_MEMORY: begin
            ...
            external_scan_index <= '0;
            external_weight_pending <= 1'b0;
            external_scan_axon <= ingress_head_axon;
            external_scan_payload <= ingress_head_payload;
          ...
          STATE_EXT_SCAN: begin
            if (base_plastic_valid_rom[external_physical_address]) begin
              if (sample_weight_valid && sample_weight_ready)
                external_weight_pending <= 1'b1;
              if (external_weight_pending && sample_weight_response_valid) begin
                work_target[external_scan_index] <= base_target_rom[external_physical_address];
                work_value[external_scan_index] <= $signed(sample_weight_response)
                  * $signed({1'b0, external_scan_payload});
                work_arrival[external_scan_index] <= current_tick
                  + base_delay_rom[external_physical_address];
                external_weight_pending <= 1'b0;
                if (external_scan_index + 1 >= work_count) state <= STATE_EXT_INSERT;
                else external_scan_index <= external_scan_index + 1'b1;
              end
            end else begin
              work_target[external_scan_index] <= base_target_rom[external_physical_address];
              work_value[external_scan_index] <= $signed(base_weight_rom[external_physical_address])
                * $signed({1'b0, external_scan_payload});
              work_arrival[external_scan_index] <= current_tick
                + base_delay_rom[external_physical_address];
              if (external_scan_index + 1 >= work_count) state <= STATE_EXT_INSERT;
        ...
        if (state == STATE_NEURON_ISSUE || state == STATE_NEURON_DRAIN) begin
          case (recurrence_engine_state)
            ...
            REC_ENGINE_SCAN: begin
              debug_recurrent_expand <= 1'b1;
              if (recurrent_plastic_valid_rom[recurrent_physical_address]) begin
                if (sample_weight_valid && sample_weight_ready)
                  recurrent_weight_pending <= 1'b1;
                if (recurrent_weight_pending && sample_weight_response_valid) begin
                  work_target[recurrent_scan_index]
                    <= recurrent_target_rom[recurrent_physical_address];
                  work_value[recurrent_scan_index] <= sample_weight_response;
                  work_arrival[recurrent_scan_index] <= current_tick + 1'b1
                    + recurrent_delay_rom[recurrent_physical_address];
                  recurrent_weight_pending <= 1'b0;
                  if (recurrent_scan_index + 1 >= work_count)
                    recurrence_engine_state <= REC_ENGINE_INSERT;
                  else recurrent_scan_index <= recurrent_scan_index + 1'b1;
                end
              end else begin
                work_target[recurrent_scan_index]
                  <= recurrent_target_rom[recurrent_physical_address];
                work_value[recurrent_scan_index]
                  <= recurrent_weight_rom[recurrent_physical_address];
                work_arrival[recurrent_scan_index] <= current_tick + 1'b1
                  + recurrent_delay_rom[recurrent_physical_address];
                if (recurrent_scan_index + 1 >= work_count)
                  recurrence_engine_state <= REC_ENGINE_INSERT;
